...
For the Dashboard left Scene workspace, open Project Assets, Models, Characters, FarmBots, Beds, and Plantings in turn. Confirm each uses the same 18rem translucent left activity overlay, the Scene retains its full width and height when it opens/closes, and its navigation or placement controls remain scrollable. The Project dropdown stays anchored beneath the Project name and renders above activity overlays. Select a Scene marker while each workspace is open and confirm its DetailsCard starts to the right of the workspace instead of covering it. Close the workspace and confirm DetailsCard returns to its normal position while the Scene dimensions and framing remain unchanged.
...
For rejected saved-marker cleanup, confirm **Remove Saved Marker** requires explicit browser confirmation and the authenticated owner-scoped DELETE removes only the selected `project_threed_markers` row. The source Sub-Module row, Project Asset assignment, other markers, credentials, and MQTT data must remain. The client must remove the warning without reloading the Canvas. Rejected source Sub-Module rows must not expose this snapshot-only deletion control.
...
- Open `/dashboard/scene?projectId=N` through a direct browser navigation. Confirm the request does not return HTTP 500 or evaluate the browser-only ThreeD runtime on the server.
...
- Open an owned active Project with no assets through a direct `/dashboard/scene?projectId=N` navigation. Confirm the established loading presentation and whole-Scene fade finish before the **Project Tour** appears.
...
For Project Bed CRUD, select a saved Bed and update width, length, height, XYZ position, degree-based Y rotation, scale, and color. Confirm the existing visual and fixed collider update without reloading the Project or remounting unrelated markers. Original assigned Beds without a saved marker must remain read-only; do not convert their marker ownership while the body is mounted. Delete a disposable saved Bed with no assigned Plantings and confirm its marker and Project assignment disappear immediately. A Dashboard-created Bed may remove its dedicated source row; an older reusable Bed must retain its source row. Assign an active Planting to a Bed and confirm Bed deletion returns a clear conflict instead of deleting or detaching the Planting.
...
- When a Project contains two or more movable Characters at the same XYZ spawn, confirm all overlapping Ecctrl markers are listed as skipped before physics, the remaining Scene stays available, and no Rapier/Rust error loop occurs. Assign unique positions before expecting those Characters to render.
...
- Open the Model Library in 2D mode, select **Place**, and confirm the view remains 2D with a crosshair over the map. Click an empty map location and confirm exactly one Model marker is created and appears at the corresponding Project position in both 2D and 3D views.
- Repeat Model placement from 3D mode and confirm the existing Scene placement path still creates exactly one marker. Start another placement, choose **Cancel**, and confirm no marker is created.
- Drag one Model Library row onto an empty 2D map location and confirm exactly one marker is created. Drag another onto the ThreeD Scene ground and confirm exactly one marker is created at the drop position. Confirm dragging outside either placement surface performs no write and **Place → click** still works.
- In 2D mode, drag an existing Project Model marker to a new map location. Confirm one position-only PATCH succeeds, the marker retains its identity, and the corresponding ThreeD visual, fixed collider, and Physics Debug outline move together without a Project refresh. Confirm Beds, Plantings, FarmBots, Characters, and traffic incidents are not draggable.
- In 3D mode, select an existing Project Model and choose **Move Model**. Confirm the cyan ground preview follows the pointer, one ground click performs one position-only PATCH, and the existing visual, fixed collider, selection, and Physics Debug outline move together without a Project refresh. Start the mode again, choose **Cancel Move**, and confirm no write occurs. Confirm normal Scene orbit and Ecctrl input remain unchanged when move mode is off.
- Run the Runtime Marker validator and confirm Project-local X/Y/Z round-trips through WGS84 latitude/longitude/altitude at equatorial, mid-latitude, and high-latitude origins, including non-zero altitude, Scene heading, and metres-per-unit scale. Invalid origins, scales, and geographic positions must fail before reaching a Map or ThreeD Scene.
- Before applying the corrective geographic migration, confirm the five origin fields are added to the parent `project` table and removed from `project_threed`, while nullable latitude/longitude/altitude remain on `project_threed_markers`. One Project may combine multiple ThreeD assignments, so `project_threed` must not own competing Scene origins.
- Configure a Project origin, create or edit one marker of each supported type, and explicitly save the Project. Confirm every affected marker keeps local X/Y/Z and receives server-derived latitude/longitude/altitude. Confirm the 2D and 3D panels resolve the same location and that Projects without an origin retain nullable GPS values without changing their existing local Scene behavior.
- Confirm `meters_per_scene_unit = 0.304800`: 100 local units must span approximately 100 feet (30.48 metres) on the Leaflet map. Projects carrying the superseded `11.119493` compatibility value must be explicitly updated before verification; local XYZ must not change. Click **Save ThreeD Project** afterward and confirm persisted marker GPS is refreshed from local XYZ.
- Select the same marker from the ThreeD Scene and the Leaflet panel. Confirm DetailsCard XYZ form values remain identical local Scene coordinates in both cases, while WGS84 latitude/longitude/altitude appears only in read-only metadata. Overlapping Leaflet marker spreading must not change the displayed GPS metadata.
- Open an owned Project in Admin and choose **Edit Project**. Under **Project Coordinate Calibration**, enter two distinct local X/Z positions and the matching surveyed latitude/longitude points, then choose **Calibrate Coordinates**. Confirm the success message reports the refreshed marker count; the Project origin, heading, and metres-per-unit change; every marker retains its exact local X/Y/Z; and saved marker GPS values are recalculated. Open `/dashboard/scene` and confirm the same marker layout now occupies the measured real-world distance and bearing in Leaflet. Equal local points, equal GPS points, incomplete values, and non-finite values must be rejected without changing the Project or its markers.
- Close and reopen **Edit Project**, then refresh the Admin Project page. Confirm all eight Point A/Point B calibration inputs reload with the saved values. A rejected calibration must retain the last successfully saved reference pairs after refresh.
- Confirm the calibration diagnostics show finite local span, WGS84 GPS span, metres-per-unit scale, and endpoint residual. Refresh the Admin page and confirm diagnostics are reconstructed from the persisted reference pairs. Recalibrate, then verify a known marker in both panels and confirm local XYZ is unchanged while the Leaflet distance and bearing remain aligned.
- Compare a north-up overhead ThreeD Scene with Leaflet at close zoom. Confirm every 2D marker uses the exact projected local XYZ pattern without artificial fan, ring, or diagonal spreading. Markers that truly share one local position must share one geographic anchor; visibility handling must not mutate their coordinates.
- After applying this coordinate convention, recalibrate the Project from its two known reference pairs. In the ThreeD Scene, orbit around the Project and confirm the true-north compass tracks the camera. Choose **Controls → North-Up View** and confirm geographic north/local -Z is at the top, east/local +X is right, south/local +Z is bottom, and west/local -X is left, matching Leaflet without reflection. Confirm marker XYZ, selection, physics, and Ecctrl controls do not change; calibration alone refreshes geographic metadata.
