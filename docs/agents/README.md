...
The current production checkpoint is [v0.19.18](../releases/v0.19.18.md), package `0.19.18`, commit `95c3a93`, deployment User-confirmed. Preserve Animation Mapping Presets, Categories, collider-aware teleport, controlled walking, explicit Character identity in live reports, persistent Canvas/physics, anchored menus and separate Garden/Ecctrl runtimes. A separate manual build result was not reported; the missing older FBX fixture remains documented. Documentation commit `d6e4068` is also User-confirmed deployed. Development is now **v0.19.19 — Character Collisions + Interactions**: follow the [staged collision/interaction plan](../plans/v0.19.19.md), beginning with evidence gathering. No new runtime feature is implemented by release preparation; schema and world-persistence expansion remain outside scope.
