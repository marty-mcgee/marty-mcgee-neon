...
Released centaur: v0.19.10c (`0.19.10-centaur`) adds OBJ bulk importing, required MTL libraries, referenced images and shared local/saved OBJ material rendering. Preserve mandatory MTL checks, inactive image deferral, MTL classification as `other` (never a thumbnail), and the staged single-Model geometry preview. Run `npm run validate:threed-obj-bundle` with the existing importer checks. See `docs/plans/v0.19.10c.md`; production deployment of `a618a66` is User-confirmed. See the completed handoff in `docs/plans/v0.19.10c-release.md`. Current candidate: **v0.19.11 — ThreeD Model Management**, package `0.19.11`, prepared but not yet production-confirmed. See `docs/plans/v0.19.11-release.md`. Preserve primary File authority, shared Texture URL reuse, refreshed Character position responses, and the original Character cylinders; generic fallback errors belong in DetailsCard.
