...
import { parseFarmBotRecoveryAcknowledgementReceipt } from './command-recovery-acknowledgement-response-core';

const ACKNOWLEDGEMENT_PATH = '/api/internal/threed-mqtt/farmbot/recoveries/acknowledgements';
const FLUSH_DELAY_MS = 250;
const REQUEST_TIMEOUT_MS = 10_000;
const INITIAL_RETRY_DELAY_MS = 1_000;
const MAX_RETRY_DELAY_MS = 60_000;

export interface FarmBotWorkerRecoveryAcknowledgementSink {
  record(acknowledgement: Readonly<FarmBotWorkerRecoveryAcknowledgement>): void;
  flush(): Promise<void>;
}

export class DisabledFarmBotWorkerRecoveryAcknowledgementSink
implements FarmBotWorkerRecoveryAcknowledgementSink {
  record(): void {}
...
function appBaseUrl(value: string): URL {
  ...
  if ((url.protocol !== 'https:' && !localHttp) || url.username || url.password
    || url.search || url.hash) {
    throw new Error('FarmBot App recovery acknowledgement URL must use HTTPS');
  }
  url.pathname = '/';
  return url;
}

export class HttpFarmBotWorkerRecoveryAcknowledgementSink
implements FarmBotWorkerRecoveryAcknowledgementSink {
  private readonly pending = new Map<string, Readonly<FarmBotWorkerRecoveryAcknowledgement>>();
  ...
  private async send(
    ...
    try {
      ...
      if (!response.ok) throw new Error('FarmBot recovery acknowledgement request failed');
...
export function createFarmBotWorkerRecoveryAcknowledgementSink(
  environment: NodeJS.ProcessEnv = process.env
): FarmBotWorkerRecoveryAcknowledgementSink {
  ...
  if (!baseUrl && !encodedHmacKey) {
    return new DisabledFarmBotWorkerRecoveryAcknowledgementSink();
  }
  if (!baseUrl || !encodedHmacKey) {
    throw new Error('FarmBot recovery acknowledgement reporting requires App URL and HMAC key');
  }
  return new HttpFarmBotWorkerRecoveryAcknowledgementSink(baseUrl, encodedHmacKey);
}
