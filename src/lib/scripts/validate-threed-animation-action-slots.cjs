...
const service = load('src/lib/services/threed/animations/slots.ts', { 'node:crypto': { randomUUID: () => 'aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa' }, 'drizzle-orm': orm, '@/lib/db/client': { db: { ...tx, transaction: fn => fn(tx) } }, '@/lib/schema/threed': schema, './contracts': contracts });
...
const schemaSource = fs.readFileSync('src/lib/schema/threed/index.ts','utf8');
