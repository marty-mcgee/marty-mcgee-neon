...
const contract = load('src/lib/services/multimedia/speech-draft.ts', () => { throw Error('Unexpected import'); });
...
const route = load('src/app/api/multimedia/speech/[id]/route.ts', name => {
  ...
  if (name === '@/lib/schema/multimedia') return { multimediaSpeech: speech };
  if (name === '@/lib/services/multimedia/speech-draft') return contract;
