...
const mocks={'next/server':{NextResponse:{json:(body,options)=>({body,status:options?.status||200})}},'@/lib/auth':{auth:async()=>signedIn?{user:{id:'owner'}}:null},'@/lib/db/client':{db},'@/lib/schema/music':{musicTracks:table('tracks'),musicAlbums:table('albums'),musicMedia:table('media')},'drizzle-orm':{eq:(...v)=>v,ne:(...v)=>v,and:(...v)=>v},'@/lib/db/sequence':{},'@aws-sdk/client-s3':{S3Client:class{async send(command){if(fail)throw Error();objects.push(command.input)}},DeleteObjectCommand:class{constructor(input){this.input=input}}}};
...
mocks['@/lib/services/music/upload-policy']=load('src/lib/services/music/upload-policy.ts');
const api=load('src/app/api/music/tracks/route.ts');
const key='threed/users/owner/multimedia/aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa/song.wav';
const track={id:7,fileUrl:'/api/music/files?key='+encodeURIComponent(key)};
const run=()=>api.DELETE({url:'https://app/api/music/tracks?id=7'});
(async()=>{
 ...
 for(let i=0;i<3;i++){queue=[[track],[],[],[]];queue[i+1]=[{id:9}];assert.equal((await run()).body.fileCleanup,'shared');}assert.equal(objects.length,1);
