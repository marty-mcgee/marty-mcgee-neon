...
const contract = load('src/lib/config/workspace-settings.ts', {});
const schema = load('src/lib/schema/settings/index.ts', { '../auth': { user: pg.pgTable('user', { id: pg.text('id') }) } });
...
const service = load('src/lib/services/settings/workspace.ts', {
  'server-only': {}, '@/lib/db/client': { db }, '@/lib/schema/settings': schema, '@/lib/config/workspace-settings': contract,
});
let session = null;
const route = load('src/app/api/settings/workspace/route.ts', {
  'next/server': { NextResponse: Response }, '@/lib/auth': { auth: async () => session },
  '@/lib/config/workspace-settings': contract, '@/lib/services/settings/workspace': service,
});
const navigation = load('src/lib/config/navigation.client.ts', {
...
(async () => {
  ...
  assert(visible.flatMap(row => row.items).some(row => row.path === '/dashboard/map'));
