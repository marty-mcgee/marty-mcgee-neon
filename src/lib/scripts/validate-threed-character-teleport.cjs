...
(async () => {
  await rapier.init();
  const { findCharacterLanding } = load('src/lib/services/threed/orchestration/teleport-landing.ts');
  ...
  const { NAVIGATION_REQUEST } = load('src/lib/services/threed/orchestration/navigation-events.ts');
