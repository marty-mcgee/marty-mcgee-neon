...
const tables={musicAlbums:table('albums',['title','artist','releaseYear','status','isPublic']),musicTracks:table('tracks',['title','albumId','duration','fileType','status']),musicMedia:table('media',['fileName','albumId','fileType','fileSize','isPrimary']),musicLinks:table('links',['title','albumId','type','status'])};
...
const out={};vm.runInNewContext(ts.transpileModule(fs.readFileSync('src/app/api/music/admin-list/route.ts','utf8'),{compilerOptions:{module:ts.ModuleKind.CommonJS,target:ts.ScriptTarget.ES2022}}).outputText,{exports:out,require:name=>{
 ...
 if(name==='@/lib/schema/music')return tables;
 if(name==='drizzle-orm')return{and:(...v)=>v,eq:(...v)=>v,count:()=>0,sql:(strings,...v)=>({strings:Array.from(strings),v})};throw Error(name);
}});
const get=query=>out.GET({nextUrl:new URL('https://app/api/music/admin-list?'+query)});
(async()=>{
 ...
 console.log('PASS: Music Admin list auth, module/sort allowlists, shared row/count owner-search scopes and paging (offline).');
