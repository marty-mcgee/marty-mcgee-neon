...
export const projectMultimedia = pgTable('project_multimedia', {
  id: serial('id').primaryKey(),
  userId: text('user_id').references(() => user.id, { onDelete: 'cascade' }),
  projectId: integer('project_id').references(() => project.id, { onDelete: 'cascade' }),
  multimediaId: integer('multimedia_id').references(() => multimedia.id, { onDelete: 'cascade' }),
  config: jsonb('config').default({}),
  isActive: boolean('is_active').default(true),
  createdAt: timestamp('created_at').defaultNow(),
  updatedAt: timestamp('updated_at').defaultNow(),
}, (table) => ({
  projectIdIdx: index('idx_project_multimedia_project_id').on(table.projectId),
  multimediaIdIdx: index('idx_project_multimedia_multimedia_id').on(table.multimediaId),
  uniqueProjectMultimedia: uniqueIndex('idx_project_multimedia_unique').on(table.projectId, table.multimediaId),
}));
...
export const projectAssets = pgTable('project_assets', {
  ...
  moduleType: text('module_type').notNull(), // ✅ NEW: 'multimedia', 'threed', 'traffic'
...
export const projectRelations = relations(project, ({ many }) => ({
  ...
  projectMultimedias: many(projectMultimedia),
