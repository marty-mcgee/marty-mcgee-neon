...
const DEFAULT_IMPORT_FILE = 'src/libraries/data/multimedia/seed-data.json';
...
async function commitImport(plan: ImportPlan, userId: string) {
  ...
  return db.transaction(async (tx) => {
    const existingAlbums = await tx
      .select({ id: multimediaAlbums.id, title: multimediaAlbums.title, artist: multimediaAlbums.artist })
      .from(multimediaAlbums)
      .where(eq(multimediaAlbums.userId, userId));
    const existingTracks = await tx
      .select({ albumId: multimediaTracks.albumId, fileUrl: multimediaTracks.fileUrl })
      .from(multimediaTracks)
      .where(eq(multimediaTracks.userId, userId));
    ...
    for (const album of plan.albums) {
      ...
      if (databaseAlbumId) {
        ...
        const [createdAlbum] = await tx
          ...
          .returning({ id: multimediaAlbums.id });
      ...
      for (const track of album.tracks) {
        ...
        await tx.insert(multimediaTracks).values({
          userId,
