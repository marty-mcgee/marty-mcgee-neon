...
import { 
  ...
  settingsDeployment,
  settingsAuditLogs
} from '@/libraries/schema/settings';
...
export type ModuleName = 'traffic' | 'threed' | 'multimedia';
...
export interface MultimediaServices {
  albums: boolean;
  tracks: boolean;
  links: boolean;
  media: boolean;
...
export interface AppSettings {
  modules: {
    ...
    multimedia: ModuleConfig<MultimediaServices>;
  };
  features: ModuleFeatures;
}
...
export function isServiceEnabled(
  module: ModuleName,
  service: keyof TrafficServices | keyof ThreeDServices | keyof MultimediaServices
): boolean {
...
const settingsPath = path.join(process.cwd(), 'src/libraries/config/settings.json');
...
if (typeof window === 'undefined') {
  ...
  console.log(`  📦 Modules: Traffic=${settings.modules.traffic.enabled}, ThreeD=${settings.modules.threed.enabled}, Multimedia=${settings.modules.multimedia.enabled}`);
  ...
  const totalServices = 
    ...
    Object.keys(settings.modules.multimedia.services).filter(k => k !== 'features' && settings.modules.multimedia.services[k as keyof typeof settings.modules.multimedia.services]).length;
