...
const source=fs.readFileSync('src/components/admin/multimedia/shared/MusicRecordsWorkspace.tsx','utf8');
...
(async()=>{
 ...
 confirmed=true;await out.run();assert.deepEqual(calls,['/api/multimedia/tracks?id=1','/api/multimedia/tracks?id=2']);assert.equal(notice,'1 deleted.');assert.equal(error,'#2: File retained');assert.equal(refreshed,1);assert.equal(lock.current,false);
 lock.current=true;await out.run();assert.equal(calls.length,2);
 console.log('PASS: actual Multimedia bulk handler confirmation, page-scoped selection, partial failures and duplicate exclusion (offline).');
