...
const contract=load('src/libraries/services/multimedia/speech-draft.ts',{});
const api=load('src/app/api/multimedia/speech/[id]/versions/route.ts',{
 ...
 '@/libraries/schema/multimedia':{multimediaSpeech:speech,multimediaSpeechVersions:versions},
 '@/libraries/services/multimedia/speech-draft':contract,
 '@/libraries/services/multimedia/upload-policy':{ownerPrefix:id=>`threed/users/${id}/multimedia/`},
 ...
 '@/app/api/multimedia/audio/generate/route':{POST:async()=>{providerCalls++;return{ok:providerOk,arrayBuffer:async()=>Buffer.from('mp3'),json:async()=>({providerStatus:402})}}},
});
