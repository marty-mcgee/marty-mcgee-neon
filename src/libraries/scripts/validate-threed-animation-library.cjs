...
const library = load('src/libraries/services/threed/animations/library.ts', {
  'drizzle-orm': orm, '@/libraries/db/client': { db }, '@/libraries/schema/threed': schema, './contracts': contracts, './slots': { validateActionSlots: async () => [], readActionSlots: async tx => tx.select().from(schema.threedAnimationActionSlots).where(orm.eq(schema.threedAnimationActionSlots.userId, 'owner')) },
});
const http = load('src/libraries/services/threed/animations/http.ts', {
  'next/server': { NextResponse: { json: (body, init) => ({ body, status: init?.status ?? 200 }) } },
  '@/libraries/auth': { auth: async () => signedIn ? { user: { id: 'owner' } } : null }, './contracts': contracts,
});
const routes = {};
for (const name of ['animations', 'animation-files', 'animation-assignments']) {
  routes[name] = load(`src/app/api/threed/${name}/route.ts`, {
    '@/libraries/services/threed/animations/http': http,
    '@/libraries/services/threed/animations/contracts': contracts,
    '@/libraries/services/threed/animations/library': library,
  });
}
...
(async () => {
  ...
  const realSchema = load('src/libraries/schema/threed/index.ts', {
