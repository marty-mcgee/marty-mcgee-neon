...
const exportsObject = {};
vm.runInNewContext(ts.transpileModule(fs.readFileSync('src/app/api/multimedia/audio/generate/route.ts', 'utf8'), { compilerOptions: { module: ts.ModuleKind.CommonJS, target: ts.ScriptTarget.ES2022 } }).outputText, {
  exports: exportsObject, Buffer, AbortSignal, process: { env },
  require: name => { if (name === 'next/server') return { NextResponse: Reply }; if (name === '@/libraries/auth') return { auth: async () => user }; throw Error(name); },
