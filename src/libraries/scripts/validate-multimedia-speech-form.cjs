...
(async()=>{
 ...
 await context.exports.run(true);assert.equal(requests[1].url,'/api/multimedia/speech/5');assert.equal(requests[1].body.revision,1);assert.equal(requests[2].url,'/api/multimedia/speech/5/versions');assert.equal(audio.url,'/api/multimedia/files?key=saved%2Fkey.mp3');
