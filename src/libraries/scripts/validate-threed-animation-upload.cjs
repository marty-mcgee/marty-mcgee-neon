...
const contracts = load(root + 'animations/contracts.ts', { '@/libraries/utils/animation': load('src/libraries/utils/animation.ts', {}) });
...
const inspector = load(root + 'animations/inspect-source.ts', { three: real,
  ...
  '@/libraries/services/threed/models/gltf-runtime-inspection-core': glb, './contracts': contracts,
});
...
const upload = load(root + 'animations/upload.ts', {
  ...
  'drizzle-orm': { and: (...args) => args, eq: (...args) => args }, '@/libraries/db/client': { db },
  '@/libraries/schema/threed': { threedAnimationFiles: { id: 'files.id', userId: 'files.owner', filePath: 'files.path' }, threedAnimations: 'clips' },
  '@/libraries/services/threed/models/model-blob-paths': paths, './contracts': contracts, './inspect-source': inspector,
});
(async () => {
