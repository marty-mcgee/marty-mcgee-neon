...
const parser = load('src/libraries/services/threed/beds/bed-list-query.ts', {});
const schema = Object.fromEntries(['threedBeds'].map(name => [name, new Proxy({}, { get: (_, column) => `${name}.${column}` })]));
schema.bedStatusEnum = { enumValues: ['active', 'pending', 'maintenance', 'dormant', 'retired'] };
...
const api = load('src/app/api/threed/beds/route.ts', {
  '@/libraries/services/threed/beds/bed-list-query': parser,
  'next/server': { NextResponse: { json: (body, init) => ({ body, status: init?.status ?? 200 }) } },
  '@/libraries/auth': { auth: async () => signedIn ? { user: { id: 'owner' } } : null }, '@/libraries/db/client': { db }, '@/libraries/schema/threed': schema, 'drizzle-orm': orm, '@/libraries/db/sequence': {},
});
async function run(query, responses = []) { queries = []; queue = responses; const result = await api.GET({ url: `http://localhost/api/threed/beds?${query}` }); assert.equal(queue.length, 0); return result; }
(async () => {
  ...
  for (const size of [1, 50, 200]) {
    ...
    const result = await run(`limit=${size}&offset=200&search=Rose&status=active&isActive=true&sort=name&direction=asc`, [[{ count: '452' }], rows]);
    ...
    assert.equal(where.args[0].args[0], 'threedBeds.userId'); assert.equal(where.args[0].args[1], 'owner');
    const search = where.args.at(-1); assert.ok(search.strings[0].startsWith('(')); assert.ok(search.strings.at(-1).endsWith(')'));
    assert.equal(queries[1].orderBy[1].args[0], 'threedBeds.id');
  }
  for (const sort of parser.BED_LIST_SORTS) for (const direction of ['asc', 'desc']) {
  ...
  const text = fs.readFileSync('src/components/admin/threed/beds/ThreeDBedsCRUD.tsx', 'utf8');
  const ast = ts.createSourceFile('beds.tsx', text, ts.ScriptTarget.Latest, true, ts.ScriptKind.TSX);
  let handler;
  ...
  const context = { beds: [{ id: 1, name: 'Rose' }, { id: 2, name: 'Mint' }], selected: new Set([1, 2, 999]), bulkBusy: false, loading: false, isSubmitting: false,
    confirm: () => true, setBulkBusy() {}, setBulkNotice: value => { notice = value; }, setSelected() {}, fetchBeds: () => refreshed++, onModuleUpdate() {},
  ...
  assert.deepEqual(requests, ['/api/threed/beds?id=1', '/api/threed/beds?id=2']); assert.match(notice, /Deleted 1 of 2/); assert.match(notice, /Rose: Protected/); assert.equal(refreshed, 1);
  console.log('PASS: bounded/authorized Bed list, grouped search, filter/count parity, every sort direction, partial bulk deletion and page-local targets');
