...
const source=fs.readFileSync('src/components/admin/multimedia/audio/SaveSpeechTrack.tsx','utf8');
...
const context={controllerRef,uploaded,albumId:'5',title:'My Speech',speech:{blob:new Blob(['audio']),text:'Original speech',voiceId:'voice'},AbortController,Error,Number,JSON,
 ...
 fetch:async(url,options)=>{calls.push({url,options});if(url==='/api/multimedia/tracks'&&failure==='network')throw Error('network');return {ok:true,status:200,json:async()=>url==='/api/multimedia/files'?(JSON.parse(options.body).action==='start'?{url:'https://s3.fixture/put',contentType:'audio/mpeg',key:'key'}:{fileUrl:'/api/multimedia/files?key=key',fileType:'audio/mpeg',fileSize:5}):{success:true,data:{id:1}}};}};
...
(async()=>{
 ...
 const data=JSON.parse(calls.at(-1).options.body);assert.equal(data.lyrics,'Original speech');assert.equal(data.metadata.voiceId,'voice');assert.equal(data.metadata.model,'s2.1-pro-free');assert.equal(data.albumId,5);assert.equal(data.fileUrl,'/api/multimedia/files?key=key');
