...
vm.runInNewContext(compile(fs.readFileSync(path.join(root, 'src/libraries/services/dashboard/project-discovery.ts'), 'utf8')), { exports: core });
const project = { id: 5, name: 'Demo Farm', description: 'Garden', slug: 'farm', isPublic: false, assetCount: '2', sceneAssetCount: '4', moduleCounts: { threed: 1, multimedia: 0, traffic: 0 } };
...
assert.equal(core.filterDashboardProjects(parsed.projects, '', 'multimedia').length, 0);
