...
const parser = load('src/libraries/services/threed/waterings/watering-list-query.ts', {});
...
const mocks = { '@/libraries/services/threed/waterings/watering-list-query': parser, 'next/server': { NextResponse: { json: (body, init) => ({ body, status: init?.status ?? 200 }) } }, '@/libraries/auth': { auth: async () => signedIn ? { user: { id: 'owner' } } : null }, '@/libraries/db/client': { db }, '@/libraries/schema/threed': schema, '@/libraries/schema/project': schema, 'drizzle-orm': orm };
(async () => {
