...
const { parsePreset, parsePresetStrategy, reviewPreset } = load('src/libraries/services/threed/animations/preset-contracts.ts', { './contracts': contracts });
...
const service = load('src/libraries/services/threed/animations/presets.ts', {
  'node:crypto': require('node:crypto'), 'drizzle-orm': orm, '@/libraries/schema/threed': schema,
  '@/libraries/db/client': { db: { ...tx, transaction: async work => { try { return await work(tx); } catch (e) { rolledBack = true; writes = []; throw e; } } } },
