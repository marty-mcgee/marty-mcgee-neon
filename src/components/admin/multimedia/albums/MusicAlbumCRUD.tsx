// components/admin/multimedia/albums/MusicAlbumCRUD.tsx
'use client';
...
import { MusicAlbum, MusicLink, MusicMedia } from '@/libraries/types/multimedia';
...
export function MusicAlbumCRUD({ onModuleUpdate }: MusicAlbumCRUDProps) {
  ...
  const fetchAlbums = async () => {
    setLoading(true);
    try {
      const response = await fetch('/api/multimedia/albums?includeTracks=true&includeLinks=true&includeMedia=true');
  ...
  const handleUpdate = async () => {
    ...
    setIsSubmitting(true);
    try {
      const response = await fetch(`/api/multimedia/albums?id=${editingAlbum.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(formData),
      });
  ...
  const handleDelete = async (id: number, title: string) => {
    ...
    try {
      const response = await fetch(`/api/multimedia/albums?id=${id}`, {
        method: 'DELETE',
      });
  ...
  const openAlbumDetail = async (album: MusicAlbum) => {
    const response = await fetch(`/api/multimedia/albums?id=${album.id}&includeTracks=true&includeLinks=true&includeMedia=true`);
    const data = await response.json();
    if (data.success) {
              ...
              {selectedAlbum && (
                <LinksManager 
                  ...
                  onLinkAdded={() => {
                    ...
                    if (selectedAlbum) {
                      fetch(`/api/multimedia/albums?id=${selectedAlbum.id}&includeLinks=true`)
                        .then(res => res.json())
                        .then(data => {
                          if (data.success) {
                            setSelectedAlbum(data.data);
              ...
              {selectedAlbum && (
                <MediaManager 
                  ...
                  onMediaChange={() => {
                    // Refresh the selected album data
                    if (selectedAlbum) {
                      fetch(`/api/multimedia/albums?id=${selectedAlbum.id}&includeMedia=true`)
                        .then(res => res.json())
                        .then(data => {
                          if (data.success) {
                            setSelectedAlbum(data.data);
