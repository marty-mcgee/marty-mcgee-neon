...
export function useMusicRecords(kind: string, enabled: boolean) {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        const response = await fetch(`/api/multimedia/admin-list?kind=${kind}&limit=${size}&offset=${page * size}&sort=${sort}&direction=${direction}&search=${encodeURIComponent(search)}`, { signal: controller.signal });
  ...
  async function removeSelected() {
    ...
    for (const row of targets) {
      try {
        const response = await fetch(`/api/multimedia/${kind}?id=${row.id}`, { method: 'DELETE' });
