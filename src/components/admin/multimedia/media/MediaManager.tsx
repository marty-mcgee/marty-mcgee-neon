// components/admin/multimedia/media/MediaManager.tsx
'use client';
...
import { MusicMedia } from '@/libraries/types/multimedia';
...
export function MediaManager({ albumId, albumTitle, onMediaChange }: MediaManagerProps) {
  ...
  const fetchMedia = async () => {
    ...
    try {
      const response = await fetch(`/api/multimedia/media?${params.toString()}`);
      if (response.ok) {
  ...
  const handleDelete = async (id: number, fileName: string) => {
    ...
    try {
      const response = await fetch(`/api/multimedia/media?id=${id}`, { method: 'DELETE' });
      if (response.ok) {
  ...
  const handleSetPrimary = async (id: number) => {
    try {
      const response = await fetch(`/api/multimedia/media/${id}/primary`, { method: 'PATCH' });
      if (response.ok) {
