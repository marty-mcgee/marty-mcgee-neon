// components/admin/multimedia/media/MusicMediaCRUD.tsx
'use client';
...
export function MusicMediaCRUD({ onModuleUpdate, pageHeader = false }: MusicMediaCRUDProps) {
  ...
  const fetchMedia = async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch('/api/multimedia/media');
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/multimedia/media?id=${editingMedia.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(payload),
      });
  ...
  const handleDelete = async (id: number, fileName: string) => {
    ...
    try {
      const response = await fetch(`/api/multimedia/media?id=${id}`, {
        method: 'DELETE',
      });
