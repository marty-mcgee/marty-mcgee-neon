...
export function SaveSpeechTrack({ speech, disabled, onBusyChange }: {
  ...
  useEffect(() => {
    ...
    void fetch(`/api/multimedia/albums?scope=owner&limit=50&offset=${page * 50}`, { signal: controller.signal })
      .then(async response => {
    ...
    try {
      if (!uploaded.current) {
        ...
        const api = async (body: object) => {
          const response = await fetch('/api/multimedia/files', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body), signal: controller.signal });
          const data = await response.json();
  ...
  return <section className="space-y-3 rounded-lg border p-3">
    ...
    {!loading && !albumError && total === 0 && <Link href="/admin/multimedia/albums" className="text-xs underline">Create an Album first</Link>}
    ...
    {(saved || uncertain) && <Link href="/admin/multimedia/tracks" className="block text-sm underline">Open Music Tracks</Link>}
  </section>;
}
