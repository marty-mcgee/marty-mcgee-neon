// components/admin/multimedia/links/MusicLinksCRUD.tsx
'use client';
...
import { MusicLink } from '@/libraries/types/multimedia';
...
export function MusicLinksCRUD({ onModuleUpdate, pageHeader = false }: MusicLinksCRUDProps) {
  ...
  const iconOptions = [
    ...
    { value: 'multimedia', label: 'Music', icon: Music },
  ...
  const fetchLinks = async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch('/api/multimedia/links');
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/multimedia/links?id=${editingLink.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(payload),
      });
  ...
  const handleDelete = async (id: number, title: string) => {
    ...
    try {
      const response = await fetch(`/api/multimedia/links?id=${id}`, {
        method: 'DELETE',
      });
