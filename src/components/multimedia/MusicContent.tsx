// components/multimedia/MusicContent.tsx - FETCH TRACKS FOR ALL ALBUMS
'use client';

import { useState, useEffect, useCallback } from 'react';
import { AlbumGrid } from '@/components/multimedia/AlbumGrid';
...
export default function MusicContent() {
  ...
  const fetchAlbums = async () => {
    try {
      ...
      const response = await fetch('/api/multimedia/albums?scope=public&includeTracks=false');
      if (response.ok) {
  ...
  const fetchTracksForAllAlbums = async (albumList: any[]) => {
    ...
    const albumPromises = albumList.map(async (album) => {
      try {
        const response = await fetch(`/api/multimedia/tracks?scope=public&albumId=${album.id}`);
        if (response.ok) {
          const data = await response.json();
          if (data.success && Array.isArray(data.data)) {
