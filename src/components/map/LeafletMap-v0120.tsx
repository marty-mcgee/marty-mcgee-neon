// components/map/LeafletMap.tsx - Fix to use gpsCenter for ThreeD markers
...
// ✅ Helper to convert 3D position to GPS
...
export function LeafletMap({
  ...
  selectedIncident,
  selectedMarker,
  center = [39.514719, -123.760382], // Default to Fort Bragg
  zoom = 12,
  height = '100%',
  gpsCenter = { lat: 39.514719, lng: -123.760382 }, // Default to Fort Bragg
}: LeafletMapProps) {
  ...
  useEffect(() => {
    ...
    if (!hasIncidents && !hasMarkers) {
      // ✅ No data - just show blank map
    ...
    // ✅ Add ThreeD markers (convert 3D position to GPS using gpsCenter)
    markers.forEach((marker) => {
      // ✅ Use GPS from metadata if available
      let lat = marker.lat;
      let lng = marker.lng;
      
      // ✅ If marker has 3D position, convert it using gpsCenter
      if (marker.metadata?.position) {
        ...
        lat = gps.lat;
        lng = gps.lng;
      } else if (marker.metadata?.gps) {
        // ✅ Use pre-calculated GPS if available
        lat = marker.metadata.gps.lat;
        lng = marker.metadata.gps.lng;
      }
        ...
        html: `
          <div class="relative">
            <div class="w-6 h-6 rounded-full flex items-center justify-center text-white text-xs font-bold shadow-lg border-2 border-white" style="background-color: ${color}">
              ${marker.type === 'plants' ? '🌿' :
                marker.type === 'plantings' ? '🌱' : 
                marker.type === 'beds' ? '🛏️' : 
                marker.type === 'characters' ? '🧑' : 
                marker.type === 'markers' ? '📍' : 
                marker.type === 'layers' ? '📐' : 
                marker.type === 'farmbots' ? '🤖' : '📍'}
            </div>
          </div>
        `,
        className: 'custom-marker',
