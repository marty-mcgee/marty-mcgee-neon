...
export function ThreeDCharacterLibraryPanel({
  ...
  return (
    <div className="threed-workspace-panel threed-scene-panel-surface absolute bottom-0 left-0 top-9 z-40 flex w-72 max-w-[calc(100vw-1rem)] flex-col overflow-hidden rounded-md border p-3 shadow-xl backdrop-blur-md">
      ...
      {placementCharacter && (
        <div className="mb-2 flex flex-wrap items-center justify-end gap-2 rounded border border-violet-500/40 [&>span]:w-full [&>span]:break-words [&>span]:[overflow-wrap:anywhere] bg-violet-500/10 p-2 text-xs">
          <span>
        ...
        {loading ? (
          ...
          return (
            <div key={character.id} className="flex items-center gap-2 rounded border p-2">
              <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded bg-violet-500/10 text-violet-600">
                {character.libraryAccess.runtime === 'ecctrl' ? '🎮' : '🧚'}
              </div>
              <div className="min-w-0 flex-1">
                <div className="break-words text-xs font-medium [overflow-wrap:anywhere]">{character.name}</div>
