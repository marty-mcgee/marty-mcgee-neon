...
export function SpeechHistory({ id, onClose, onChange }: { id: number; onClose: () => void; onChange: () => void }) {
  ...
  return <Dialog open onOpenChange={open => { if (!open && !busy) onClose(); }}><DialogContent className="w-[calc(100vw-2rem)] sm:max-w-3xl max-h-[90dvh] overflow-y-auto">
    ...
    {versions.map(version => <div key={version.id} className="space-y-2 rounded border p-3 text-sm">
      ...
      {version.status === 'ready' && version.storageKey && <><audio controls preload="none" className="w-full" src={`/api/multimedia/files?key=${encodeURIComponent(version.storageKey)}`} /><Button size="sm" variant="outline" disabled={busy || loading || uncertain || !!record?.archivedAt || record?.acceptedVersionNumber === version.versionNumber} onClick={() => action(version.versionNumber)}>Accept version {version.versionNumber}</Button></>}
