// app/admin/multimedia/albums/[id]/page.tsx
'use client';
...
import { MediaManager } from '@/components/admin/multimedia/media/MediaManager';
...
export default function AlbumDetailPage() {
  ...
  const fetchAlbum = async () => {
    try {
      const response = await fetch(`/api/multimedia/albums?id=${albumId}`);
  ...
  const fetchTracks = async () => {
    setLoading(true);
    try {
      const response = await fetch(`/api/multimedia/tracks?albumId=${albumId}`);
      const data = await response.json();
      if (data.success) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = '/api/multimedia/tracks';
  ...
  if (!album) {
    return (
      <div className="text-center py-12">
        ...
        <Button variant="outline" className="mt-4" onClick={() => router.push('/admin/multimedia/albums')}>
          Back to Albums
        </Button>
      </div>
    );
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        <Button variant="ghost" size="sm" onClick={() => router.push('/admin/multimedia/albums')}>
