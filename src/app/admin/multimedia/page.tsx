...
export default function AdminMusicDashboard() {
  ...
  const fetchStats = async () => {
    try {
      const response = await fetch('/api/multimedia/admin/stats');
      if (response.ok) {
                ...
                <div key={track.id} className="flex items-center justify-between p-2 rounded-lg hover:bg-accent/50">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Button size="sm" variant="ghost" asChild>
                      <Link href={`/admin/multimedia/tracks/${track.id}`}>
        ...
        <CardContent className="space-y-3">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            <Button asChild className="justify-start">
              <Link href="/admin/multimedia/albums">
                <Album className="h-4 w-4 mr-2" />
                Create New Album
              </Link>
            </Button>
            <Button asChild variant="outline" className="justify-start">
              <Link href="/admin/multimedia/tracks">
                <Music className="h-4 w-4 mr-2" />
                Add New Track
              </Link>
            </Button>
            <Button asChild variant="outline" className="justify-start">
              <Link href="/admin/multimedia/links">
