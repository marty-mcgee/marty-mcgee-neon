...
import { musicAlbums, musicTracks, musicMedia, musicLinks } from '@/lib/schema/music';
import { and, eq, sql, count } from 'drizzle-orm';

const tables = { albums: musicAlbums, tracks: musicTracks, media: musicMedia, links: musicLinks };
const fields = {
...
export async function GET(request: NextRequest) {
  ...
  const name = key === 'media' ? musicMedia.fileName : (table as typeof musicAlbums).title;
