...
async function readBoundedBody(request: Request): Promise<Buffer> {
  ...
  while (true) {
    ...
    if (length > MAX_FARMBOT_RECOVERY_ACKNOWLEDGEMENT_BYTES) {
      await reader.cancel();
...
export async function POST(request: Request) {
  ...
  if (!encodedKey) {
    return json({ success: false, error: 'FarmBot recovery ingestion is not configured' }, 503);
  }
  ...
  try {
    ...
    const acknowledgement = parseFarmBotRecoveryAcknowledgement(
    ...
    const updated = await recordFarmBotCommandRecoveryAcknowledgement({
      userId: acknowledgement.ownerId,
      commandId: acknowledgement.commandId,
      rpcLabel: acknowledgement.recoveryRpcLabel,
      outcome: acknowledgement.state === 'confirmed' ? 'ok' : 'error',
      now: acknowledgement.receivedAt,
    });
    return json({
      success: true,
      data: { commandId: updated.commandId, recoveryState: updated.recoveryState },
    }, 202);
  } catch (error) {
    if (error instanceof FarmBotRecoveryAcknowledgementInputError
      || error instanceof SyntaxError) {
      return json({ success: false, error: 'Invalid FarmBot recovery acknowledgement' }, 400);
    }
    ...
    if (error instanceof FarmBotCommandTransitionConflictError) {
      return json({ success: false, error: 'FarmBot recovery state conflict' }, 409);
    }
    if (error instanceof MqttWorkerAuthError) {
      return json({ success: false, error: 'Unauthorized' }, 401);
    }
    console.error('FarmBot recovery acknowledgement ingestion failed', {
      errorName: error instanceof Error ? error.name : 'UnknownError',
    });
    return json({ success: false, error: 'FarmBot recovery acknowledgement failed' }, 500);
  }
}
