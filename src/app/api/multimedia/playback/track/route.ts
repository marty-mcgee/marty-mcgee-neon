...
import { multimediaPlaybackHistory } from '@/libraries/schema';
...
export async function POST(request: NextRequest) {
  try {
    ...
    await db.insert(multimediaPlaybackHistory).values({
    ...
    await db.execute(
      sql`UPDATE multimedia_tracks SET play_count = play_count + 1 WHERE id = ${trackId}`
    );
