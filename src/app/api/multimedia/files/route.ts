...
import { ownerPrefix, ownsMediaKey, uploadPolicy } from '@/libraries/services/multimedia/upload-policy';
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (body.action === 'complete') {
      ...
      return NextResponse.json({ fileUrl: `/api/multimedia/files?key=${encodeURIComponent(body.key)}`, fileType: policy.contentType, fileSize: object.ContentLength }, { headers });
    }
