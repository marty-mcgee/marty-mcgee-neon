// app/api/multimedia/links/route.ts
import { NextRequest, NextResponse } from 'next/server';
import { auth } from '@/libraries/auth';
import { db } from '@/libraries/db/client';
import { multimediaLinks } from '@/libraries/schema/multimedia';
import { eq, and, desc, sql } from 'drizzle-orm';
import { ensureTableSequence } from '@/libraries/db/sequence';

// ============================================
// GET /api/multimedia/links
// Query Parameters:
  ...
  try {
    ...
    if (id) {
      ...
      const [link] = await db
        ...
        .where(
          and(
            eq(multimediaLinks.id, parsedId),
    ...
    const linkScope = and(
      eq(multimediaLinks.userId, userId),
      parsedAlbumId ? eq(multimediaLinks.albumId, parsedAlbumId) : undefined,
      parsedTrackId ? eq(multimediaLinks.trackId, parsedTrackId) : undefined,
      independent
        ? and(
            sql`${multimediaLinks.albumId} IS NULL`,
            sql`${multimediaLinks.trackId} IS NULL`
          )
        : undefined
    );
    ...
    const links = await query.orderBy(
      desc(multimediaLinks.displayOrder),
      desc(multimediaLinks.createdAt)
    );
...
// ============================================
// POST /api/multimedia/links - Create a new link
// ============================================
export async function POST(request: NextRequest) {
  try {
    ...
    // ✅ Ensure sequence
    await ensureTableSequence('multimedia_links');

    // ✅ Create the link
    const [newLink] = await db
      .insert(multimediaLinks)
      .values({
        userId,
        title,
        url,
...
// ============================================
// PUT /api/multimedia/links - Update a link
// ============================================
...
// ============================================
// DELETE /api/multimedia/links - Delete a link
// ============================================
