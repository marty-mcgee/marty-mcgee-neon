...
import { multimediaAlbums, multimediaTracks, multimediaLinks } from '@/libraries/schema';
import { MusicLinkType, AlbumStatus, TrackStatus } from '@/libraries/types/multimedia';
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Seed sample albums
    const albums = await db.insert(multimediaAlbums).values([
      {
    ...
    await db.insert(multimediaLinks).values([
      {
        userId: session.user.id,
        title: 'Spotify',
