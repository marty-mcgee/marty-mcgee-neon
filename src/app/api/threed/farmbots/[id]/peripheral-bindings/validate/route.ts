...
export async function GET(request: Request, context: RouteContext) {
  ...
  try {
    const validation = await validateFarmBotPeripheralBinding(
      session.user.id,
      Number(id),
      semanticAction
    );
    ...
    if (error instanceof FarmBotPeripheralBindingNotFoundError) {
      return json({ success: false, error: 'FarmBot peripheral binding is not configured' }, 409);
    }
    ...
    console.error('FarmBot peripheral binding validation failed', {
      errorName: error instanceof Error ? error.name : 'UnknownError',
    });
    return json({ success: false, error: 'FarmBot peripheral binding validation failed' }, 500);
  }
}
