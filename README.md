...
The approved **ThreeD FarmBot Integration Plan** continues from the v0.18.3a production checkpoint:
...
| v0.18.3b | Phase 4 — Emergency delivery safety boundary | Release candidate: owner-scoped emergency audit persistence, strict disabled worker endpoint, shared arbitration, exact RPC correlation, authenticated acknowledgement ingestion, and queued acknowledgement reporting through Phase 4L-K. Executors and MQTT publishing remain disabled. |
...
Phases 2A–2D, the ThreeD MQTT control layer, and Phase 3 are released and production-verified. Phase 4 work through the v0.18.3b release candidate adds guarded delivery and acknowledgement foundations while keeping every production executor and MQTT publishing disabled. No physical operation is enabled.
